	...
	<div class="d-flex align-items-center justify-content-center w-100 h-100">
		<div class="card shadow form-auth">
			<div class="card-title text-center border-bottom">
				<h2 class="p-3">{{ $page == "reg" ? "Registration" : "Auth" }}</h2>
			</div>
			<div class="card-body mb-2">
				...
				@endif
				<form action="{{ $page == 'reg' ? 'auth/register' : 'auth/login' }}" method="POST">
					@csrf
					<div class="mb-4">
					...
					<div class="mb-5">
						...
						<a href="{{ $page == 'reg' ? 'auth' : 'reg' }}" class="d-block float-end text-primary mt-2">
							{{ $page == "reg" ? "Sign in" : "Sign up" }}
						</a>
					</div>
				</form>
			</div>
