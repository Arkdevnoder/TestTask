	...
	<div class="wrapper">
			...
			<section class="sidebar">
				...
				<div class="user-panel">
					<div class="pull-left image">
						<img src="{{ asset("/bower_components/admin-lte/dist/img/p.jpg") }}" class="img-circle" alt="User Image" />
					</div>
					<div class="pull-left info">
						<p>({{ ucfirst($role) }}) {{ Auth::user()->login }}</p>
						<!-- Status -->
		...
		<div class="content-wrapper">
			...
			<section class="content-header">
				<h1>
					{{ $page == 'Dashboard' ? 'Your watchlist' : '' }}
					{{ $page == 'Children' ? 'Add your child' : '' }}
					{{ $page == 'About' ? 'About our system' : '' }}
				</h1>
				<ol class="breadcrumb">
...
This is a kind of CRM system that can help the company to establish its work. Good luck.
				...
				@if ($page == 'Dashboard' && $role == 'parent')
					...
					@endphp
					<div class="row">
						<div class="col-xs-12 col-md-{{7+1}}">
									...
									<table class="table table-striped table-responsive">
										...
										<tbody>
											@foreach ($data as $task)
												<tr>
													<td>
														{{ App\Models\User::where('id', '=', $task['child'])->first()->login }}
													</td>
													<td data-ttextable-id="{{ $task['id'] }}">{{ $task['text'] }}</td>
													<td data-status-id="{{ $task['id'] }}" class="text-right">{{ $task['status'] }}</td>
													<td class="text-right">
														<span data-countable-id="{{ $task['id'] }}">{{ $task['rate'] }}</span> <i class="fa fa-star-o"></i>
													</td>
													<td class="text-right">
														<button type="button" data-id="{{ $task['id'] }}" data-toggle="modal" data-target="#modal-default" class="btn btn-block btn-default">
															Edit your task
														</button>
													</td>
												</tr>
				...
				@if ($page == 'Dashboard' && $role == 'children')
					@php
						$list = App\Http\Controllers\GetTaskController::list();
						$links = $list['links'];
						$data = $list['data'];
										...
										<tbody>
											@foreach ($data as $task)
												<tr>
													...
													<td class="text-right">{{ $task['rate'] }} <i class="fa fa-star-o"></i></td>
												</tr>
											@endforeach
										</tbody>
									</table>
						...
						<div class="col-xs-12 col-md-4">
							<div class="box box-success">
								...
								<div class="box-body">
									...
									<input name="id" disabled class="form-control m" type="text" name="rate" placeholder="Child's ID" value="{{ $hash }}">
								</div>
							</div>
						</div>
					</div>
